...
class CheckIn {
	...
	function startService(){
		...
		while (1){
            ...
            setproctitle('mediasync-checkind: Last Check-In - ' . date('g:i:s A'));
	...
	function performCheckIn(){
		...
		$xml->addChild('version', $mediaSync->version);
		...
		if (!$response){
			$mediaSync->err->logError('Unable to perform checkin - Connection Error', 'mediasync_checkind');
		} else {
			if ($response->status['code'] != '200'){
				$mediaSync->err->logError('Invalid XML Response - ' . $response->asXML(), 'mediasync_checkind');
			}
		}
	}
